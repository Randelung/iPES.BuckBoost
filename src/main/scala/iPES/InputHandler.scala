...
class InputHandler(context: CustomContext, animator: Animator) {
  ...
  def eventHandler(e: dom.MouseEvent): Unit = {
    ...
    items foreach (i => {
      ...
      {
        item.onClick(Vector2D(e.pageX - context.canvas.offsetLeft, e.pageY - context.canvas.offsetTop) - item.a)
